...
def render_card(c):
    ...
    alert = html.Div(
        ...
        style={'color': 'red', 'fontWeight': 'bold', 'fontSize': '24px', 'marginBottom': '12px'}
    ) if severity_val >= 30 else None

    def line(text, size=22, margin=8):
        return html.Div(text, style={'fontSize': f'{size}px', 'marginBottom': f'{margin}px'})

    card_fields = [
        line(f"📸 Image: {c['Image_Name']}", 24),
    ...
    return html.Div([
        html.Div([
            ...
            # Right: Image
            html.Img(src=c['Image_Encoded'], style={
                'width': '550px',
                'maxHeight': '550px',
                'objectFit': 'contain',
                'borderRadius': '10px',
...
# === Layout ===
app.layout = html.Div([
    html.H1("🛠 Real-Time Prescriptive Maintenance Log", style={'textAlign': 'center', 'fontSize': '36px'}),
    html.Hr(),

    html.Div([
        html.Label("Filter:", style={'fontSize': '22px'}),
        dcc.RadioItems(
            id='filter-option',
            options=[
                {'label': 'All', 'value': 'all'},
...
def update_dashboard(n, filter_value, current_index, shown_cards_data):
    df = load_data()
    if df.empty:
        return [html.Div("⚠️ No data available.", style={'fontSize': '22px'})], current_index, shown_cards_data
    ...
    if current_index >= len(df):
        ...
        done_message = html.Div("✅ All images have been shown.", style={
            'textAlign': 'center', 'color': 'green', 'fontSize': '24px', 'margin': '10px'
        })
